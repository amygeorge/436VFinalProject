...
function makeChart3(_data) {
  d3.selectAll(".race-selection-button").on("click", function () {
    ...
    if (this.id === "chart3-race-All") {
      ...
      d3.select("#chart3-race-Non-Caucasian").classed(
        "mdc-button--raised",
        false
      );
      ...
      if (this.id === "chart3-race-Caucasian") {
        ...
        d3.select("#chart3-race-Caucasian").classed(
          "mdc-button--raised",
          false
        );
      }
    ...
    chart3.data = prepChart3Data(_data);
    chart3.updateVis();
  });
  ...
  d3.selectAll(".chart3-selection").on("click", function () {
    d3.select(`#chart3-selection-${chart3.config.selection}`).classed(
      "mdc-button--raised",
      false
    );
    d3.select(`#${this.id}`).classed("mdc-button--raised", true);
...
function prepChart3Data(_data) {
  let data = chart3Filters.race
    ? _data.filter((f) => f.race === chart3Filters.race)
    : _data;
